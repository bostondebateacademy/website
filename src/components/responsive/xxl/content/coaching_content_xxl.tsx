...
export default function CoachingContentXXL() {
    const [heightKitBox, setHeightKitBox] = useState(0)
    const refKitBox = useRef<HTMLDivElement>(null)

    const [buttonX, setButtonX] = useState(0)
    const [buttonY, setButtonY] = useState(0)
    const btnref = useRef<HTMLDivElement>(null)

    const [heightSkillsCard, setHeightSkillsCard] = useState(0)
    const refSkillsCard = useRef<HTMLDivElement>(null)

    useLayoutEffect(() => {
        if(refKitBox.current) {
            setHeightKitBox(refKitBox.current.clientHeight);
        }
        if(btnref.current) {
            setButtonX(btnref.current.offsetLeft);
            setButtonY(btnref.current.offsetTop);
        }
        if(refSkillsCard.current) {
            setHeightSkillsCard(refSkillsCard.current.clientHeight);
        }
    }, []);
